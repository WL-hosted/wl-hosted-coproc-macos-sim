...
int wlh_sim_write_hello(int fd, const wlh_sim_hello_t *hello) {
    ...
    // clang-format off
    if (hello == NULL ||
        hello->role < WLH_SIM_ROLE_HOST ||
        hello->role > WLH_SIM_ROLE_MANAGER ||
        (hello->flags & (uint8_t)~WLH_SIM_FLAG_SIDEBAND) != 0u ||
        hello->max_record_size < 8u ||
        hello->max_record_size > WLH_SIM_MAX_RECORD_SIZE)
        return -1;
    // clang-format on
...
int wlh_sim_read_hello(int fd, wlh_sim_hello_t *hello) {
    ...
    if (hello == NULL ||
        ...
        memcmp(bytes, hello_magic, sizeof(hello_magic)) != 0 ||
        bytes[8] != 1u ||
        bytes[9] != 0u ||
        bytes[10] < WLH_SIM_ROLE_HOST ||
        bytes[10] > WLH_SIM_ROLE_MANAGER ||
        (bytes[11] & (uint8_t)~WLH_SIM_FLAG_SIDEBAND) != 0u)
        return -1;
    // clang-format on
    ...
    // clang-format off
    return hello->max_record_size >= 8u &&
                   hello->max_record_size <= WLH_SIM_MAX_RECORD_SIZE
               ? 0
               : -1;
    // clang-format on
...
int wlh_sim_write_record(
    ...
    if (kind < WLH_SIM_RECORD_WIRE_FRAME ||
        kind > WLH_SIM_RECORD_FAULT_RESPONSE ||
        (payload == NULL && payload_size != 0u) ||
        max_record_size < 8u ||
        payload_size > (size_t)UINT32_MAX - 4u ||
        payload_size > (size_t)max_record_size - 8u)
        return -1;
    // clang-format on
...
int wlh_sim_read_record(
    ...
    // clang-format off
    if (record_len < 4u ||
        record_len > max_record_size - 4u ||
        record_len - 4u > capacity ||
        header[4] < WLH_SIM_RECORD_WIRE_FRAME ||
        header[4] > WLH_SIM_RECORD_FAULT_RESPONSE ||
        header[5] != 0u ||
        header[6] != 0u ||
        header[7] != 0u)
        return -1;
