...
- CMake >= 3.20
- C11 编译器（Clang 或 GCC）
- Swift 工具链（Xcode Command Line Tools，真实 Wi-Fi 后端需要）
- Ninja（混合 C + Swift target 不支持 Unix Makefiles 生成器）
- pthread

除 Foundation / CoreWLAN 系统框架外无需额外第三方库。

## 3. 构建步骤

推荐 out-of-tree 构建，并打开测试（必须使用 Ninja 生成器）：
...
--scenario happy|auth-fail|ap-not-found
    指定 mock Wi-Fi 场景，默认 happy。仅 mock 后端有效；与 --backend real 同用时被忽略并打印警告。

--backend mock|real
    选择 Wi-Fi 后端，默认 mock。real 仅在 macOS 上可用（见 6.1 节）；其他平台指定 real 会以退出码 2 拒绝。
...
## 6.1 真实 Wi-Fi 后端（--backend real）

`--backend real` 用 CoreWLAN 操作本机真实 Wi-Fi（Swift 实现，见 `src/wifi_backend_real.swift` 与 C 适配层 `src/wifi_backend_real.c`）：

- **扫描**：`CWInterface.scanForNetworks(withName:)` 返回本机周围的真实 BSS（SSID、BSSID、security、channel、RSSI）。
- **连接**：`CWInterface.associate(to:password:)` 真实关联到目标 AP——**会改变本机 Wi-Fi 关联状态**；找不到 AP 上报 reason 2，关联失败上报 reason 3。security 不做预校验，由真实 AP 裁决。
- **断开**：`CWInterface.disassociate()`，**会断开本机当前 Wi-Fi 连接**。
- **数据面**：与 mock 相同，仍是以太网 echo，不经过真实网络。

注意事项：

- macOS 14+ 上 CoreWLAN 需要运行进程（及其终端/App）拥有"定位服务"权限，否则扫描结果中的 SSID 会被系统隐藏，可能导致扫描结果为空或连接时找不到目标 AP。
- `--scenario` 与 `WIFI_SCAN_FAILURE` 故障注入仅 mock 后端有效；real 下 `WIFI_SCAN_FAILURE` 响应 not supported。`WIFI_DISCONNECT` 在 real 下会真实执行 `disassociate()`。
- real 后端不监听 CoreWLAN 异步事件：本机 Wi-Fi 被外部因素（关 Wi-Fi、信号丢失）断开时不会主动上报 `disconnected`。
- mock 的 25 ms 模拟时延在 real 下为 0；CoreWLAN 调用本身在 `wifi_worker` 任务上同步阻塞执行。
...
- **Wi-Fi**：`wifi_initialize/scan/connect/disconnect` 将请求投递到有界 Wi-Fi 队列，由 `wifi_worker` 异步执行；具体行为经 `wlh_wifi_backend_t` vtable 分发到 mock 或 real（CoreWLAN）后端，见 `--backend` 参数。
...
- **Wi-Fi 任务（`wifi_worker`）**：初始化、扫描、连接、断开请求通过深度 16 的有界队列序列化执行；mock 后端每个操作先 `sleep_ms(backend_delay_ms)`（默认 25 ms）再上报结果，real 后端时延为 0，CoreWLAN 调用在该任务上同步执行。
...
| `WIFI_DISCONNECT` | 上报一次非本地触发的断开；real 后端会先真实执行 `disassociate()`。 |
| `WIFI_SCAN_FAILURE` | 让后续扫描直接失败（仅 mock 后端；real 后端响应 not supported）。 |
...
真实后端（`--backend real`）依赖本机 Wi-Fi 硬件与定位权限，无法纳入自动化测试，需手动验证：

1. 确认已授予终端"定位服务"权限（系统设置 → 隐私与安全性 → 定位服务）；
2. 以 `--backend real` 启动本程序并与 Host Sim 连通；
3. 扫描应返回本机周围真实 BSS；
4. 连接一个真实 AP 前后用 `networksetup -getairportnetwork en0` 对比本机关联状态；
5. 断开后确认本机 Wi-Fi 已解除关联；
6. 以太网 echo 与 mock 模式行为一致。
...
| 4 | OSAL / 任务 / Core 初始化失败，或 real 后端创建失败。 |
| 5 | 运行时缓冲区分配失败。 |
